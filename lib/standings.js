...
/**
 * Whether a table is a ranking yet, or still an arbitrary ordering.
 *
 * A single played match makes `points` non-zero for two players and leaves the
 * rest on nothing, so every position below them is decided by the tiebreak
 * chain's final seeded draw — which is stable but meaningless. Presenting that
 * as "2nd, qualifying" to someone who has not played is simply false.
 *
 * Once everyone has had a game the order reflects results, so it can carry
 * positions and a qualification highlight. For this league that is after round
 * one, since each round gives every player in a group exactly one fixture.
 */
export function isRanked(rows) {
  return rows.length > 0 && rows.every((row) => row.played > 0);
}
...
export function computeTable(playerIds, fixtures, results, rules, drawSeed) {
  ...
  // Sort first: the draw order must be a function of the id SET and the seed,
  // never of the order a caller happened to pass them in. Otherwise the site
  // and the Slack bot could rank a tie differently.
  const drawOrder = new Map(
    seededShuffle([...playerIds].sort(), drawSeed).map((id, index) => [id, index]),
  );
