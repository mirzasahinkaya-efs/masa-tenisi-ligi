...
/**
 * How many games win a match at this stage. The group stage and the playoffs are
 * played to different lengths, so nothing can judge a score without first
 * knowing which fixture it belongs to.
 *
 * Returns undefined for an unknown stage rather than falling back to a default —
 * a guess here would validate a playoff result against the group rule.
 */
export const gamesToWinFor = (rules, stage) => rules?.gamesToWin?.[stage];

/** The largest format across all stages, for offering score options up front. */
export const maxGamesToWin = (rules) => {
  const values = Object.values(rules?.gamesToWin ?? {}).filter(Number.isInteger);
