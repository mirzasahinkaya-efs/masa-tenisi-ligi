...
/**
 * Whether every group has finished. The cross-group fourth place cannot be
 * compared until then — mid-season the two candidates have played different
 * numbers of matches, so it is not like-for-like — and the site's own footnote
 * promises exactly this. Shared so the bracket and the table agree rather than
 * each carrying its own copy of the condition.
 */
export function allGroupsComplete(tables, fixtures, results) {
  const names = Object.keys(tables);
  // No groups is not "all complete": that would publish a fabricated qualifier.
  return names.length > 0 && names.every((name) => isGroupComplete(name, fixtures, results));
}
 ...
 *
 * Returns null unless at least two groups actually have a fourth place. It does
 * not check whether the groups are finished — callers do, via
 * allGroupsComplete. Both the bracket and the standings table gate on that,
 * because a mid-season comparison would rank two players who have played
...
export function resolveBracket(tables, fixtures, results, { drawSeed } = {}) {
  ...
  const bestFourth = allGroupsComplete(tables, fixtures, results)
