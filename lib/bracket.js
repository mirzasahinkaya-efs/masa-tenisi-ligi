 ...
 * then game difference, then games won, then the same seeded draw the tables
 * use, so a dead heat resolves reproducibly from the committed seed instead of
 * by whichever group happened to be iterated first.
 *
...
export function bestFourthPlayerId(tables, drawSeed) {
  ...
  const ids = fourths.map((row) => row.playerId).sort();
  // An absent or non-numeric seed would make mulberry32 produce NaN and the
  // comparison arbitrary, so fall back to id order — still deterministic.
  const order = new Map(
    (Number.isFinite(drawSeed) ? seededShuffle(ids, drawSeed) : ids)
