...
// Only now, because the group stage and the playoffs are played to different
// lengths — the legal scores depend on which fixture this turned out to be.
const gamesToWin = gamesToWinFor(league.rules, fixture.stage);
const parsed = parseScore(scoreText, { gamesToWin });
if (!parsed.ok) {
  if (parsed.error === 'NO_RULE') {
    console.error(`data/league.json has no rules.gamesToWin.${fixture.stage}, so ${fixture.id} cannot be checked.`);
  } else if (parsed.error === 'FORMAT') {
    console.error(`Could not parse "${scoreText}" as a score. Expected a shape like "${gamesToWin}-1".`);
  } else {
    console.error(
      `Illegal score "${scoreText}" for ${fixture.id} (${fixture.stage}): `
